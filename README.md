...
| [`telemetry/`](telemetry) | telemetry.evilbyte.net — pulls real Cloudflare traffic from Log Explorer every 5 minutes and rates it with the same formula, for validating the formula against reality |
...
| [`scripts/`](scripts) | Builds the submission-ready RFCXML from the Markdown source, and checks the PDF against it |
